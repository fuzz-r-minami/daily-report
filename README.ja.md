...
Git / SVN / Perforce / Jira / Redmine / Confluence / Slack / Google Calendar から作業履歴を自動収集し、日報・週報・月報を生成する Windows デスクトップアプリです。
...
2. プロジェクトに使っているツール（Git / SVN / Perforce / Jira / Redmine / Confluence / Slack など）を設定する
...
| 項目 | 説明 |
|------|------|
| Jira URL | Jira インスタンスの URL（例: `https://company.atlassian.net`） |
| メールアドレス / ユーザー名 | Atlassian アカウントのメールアドレス（Cloud）またはユーザー名（Server） |
| API トークン | Atlassian アカウント設定で発行できます |
| プロジェクトキー | 特定プロジェクトに絞る場合に入力（例: `PROJ`）、空の場合は全プロジェクトが対象 |
| Server / Data Center モード | オンプレミスの Jira を使う場合に有効にする |
...
| 項目 | 説明 |
|------|------|
| Confluence URL | Confluence インスタンスの URL（例: `https://company.atlassian.net`） |
| メールアドレス / ユーザー名 | Atlassian アカウントのメールアドレス（Cloud）またはユーザー名（Server） |
| API トークン | Atlassian アカウント設定で発行できます |
| スペースキー | 特定スペースに絞る場合に入力（例: `TEAM`）、空の場合は全スペースが対象 |
| Server / Data Center モード | オンプレミスの Confluence を使う場合に有効にする |

収集されるのは、指定期間内に自分が**作成または更新したページ**です。
...
Git / SVN / Perforce / Jira / Redmine / Confluence / Slack / Google Calendar に活動があった日を稼働日とし、複数プロジェクトで同じ日に活動があった場合は 1/N 日として按分します。
